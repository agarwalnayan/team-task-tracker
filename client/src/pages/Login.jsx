...
export default function Login({ dark, setDark }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(getRequestError(err, 'Sign-in failed'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 relative flex items-center justify-center px-4 py-12 font-sans antialiased">
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-2 text-sm">Sign in to TaskFlow</p>
        </div>

        <div className="rounded-2xl bg-white dark:bg-slate-900 p-8 shadow-2xl shadow-black/40 ring-1 ring-slate-200/50 dark:ring-slate-700/80">
          {error && (
            <div
              role="alert"
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-shadow"
              />
            </div>
            <div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-semibold shadow-lg shadow-blue-600/25 transition-colors"
            >
